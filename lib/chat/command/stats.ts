...
export const stats: CommandHandler = {
    ...
    execute: async (client, io, state, args) => {
        ...
        try {
            ...
            handlerLogger.error('Failed to get player stats for', playerName, game, platform, error instanceof Error ? error.message : error);
...
export const link: CommandHandler = {
    identifier: 'link',
    aliases: [ 'profile' ],
    permittedRoles: [ Role.Viewer ],
    execute: async (client, io, state, args) => {
        ...
        if (!playerName) {
            await client.say(Config.SPECTATOR_CHANNEL, 'Usage: !link [player name] [[game]] [[platform]]');
            return;
        }
        ...
        try {
            ...
            handlerLogger.error('Failed to get profile link for', playerName, game, platform, error instanceof Error ? error.message : error);
            response = `Sorry, failed to fetch profile link for ${playerName}`;
        }
...
export const summary: CommandHandler = {
    ...
    execute: async (client, io, state, args) => {
        ...
        try {
            ...
            handlerLogger.error('Failed to get live stats for', playerName, error instanceof Error ? error.message : error);
...
export const active: CommandHandler = {
    ...
    execute: async (client) => {
        ...
        try {
            ...
            handlerLogger.error('Failed to get live concurrent player stats', error instanceof Error ? error.message : error);
